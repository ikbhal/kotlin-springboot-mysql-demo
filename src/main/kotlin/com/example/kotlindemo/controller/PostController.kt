...
class PostController(private val postRepository: PostRepository) {
    ...
    @PostMapping("/posts")
    fun createNewPost( @RequestBody post: Post): Post = postRepository.save(post)
    ...
    fun updatePostById(@PathVariable(value = "id") postId: Long,
                        @RequestBody newPost: Post): ResponseEntity<Post> {
        return postRepository.findById(postId).map { existingPost ->
            val updatedPost: Post = existingPost.copy(title = newPost.title, content = newPost.content)
